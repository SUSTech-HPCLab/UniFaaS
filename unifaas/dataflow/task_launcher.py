...
class TaskLauncher:
    ...
    def __init__(self, executors, launch_function, scheduling_strategy,task_status_tracker):
    ...
    def no_block_launch(self):
        for key in self.launch_que_dict.keys():
            ...
            while len(dq) > 0:
                task_record = dq.pop()
                self.resource_poller.update_when_launch_task(key,task_record=task_record)
                self.task_status_tracker.modify_ready_to_launch(-1)
    ...
    def change_dest_executor(self, task_record, dest_executor):
        ...
        exp_logger.info(f"[TaskLauncher] task {task_record['id']} is moved from {orginal_executor} to {dest_executor}")
        self.task_status_tracker.modify_ready_to_launch(-1)
        task_record['executor'] = dest_executor
        task_record['changed_executor'] = True
        self.resource_poller.update_status_when_submit_one_task(dest_executor, task_record=task_record)
    ...
    def block_launch(self):
        ...
        for key in self.launch_que_dict.keys():
            ...
            while len(dq) > 0:
                cur_running_task_num = resource_status[key]['total_workers'] - resource_status[key]['idle_workers'] + resource_status[key]['pending_tasks']
        ...
        for key in self.launch_que_dict.keys():
            ...
            for possible_ep in self.launch_que_dict.keys():
                if key != possible_ep:
                    upper_bound_iter += max(10, resource_status[possible_ep]['idle_workers'] - \
                        resource_status[possible_ep]['pending_tasks'] - resource_status[possible_ep]['launch_queue_task_num'])
            tmp_deque = deque()
            while upper_bound_iter > 0:
                ...
                if 'changed_executor' in task_record.keys():
                    if task_record['changed_executor']:
                        ...
                        upper_bound_iter -= 1
                        continue
                cur_cost = self.resource_poller.calculate_ideal_execution_duration(key, task_record)
                ...
                for ep in feasible_ep:
                    ...
                    new_cost = self.transfer_predictor.real_time_predict_comm_cost_for_task_record(task_record, ep) + \
                        self.resource_poller.calculate_ideal_execution_duration(ep, task_record)
                    if new_cost < cur_cost* 0.7 and abs(cur_cost - new_cost) > 300: #tmp
                        min_ep = ep
                        cur_cost = new_cost
                if min_ep != key:
                    exp_logger.info(f"cur cost {cur_cost_log} higher than new cost {cur_cost}")
    ...
    def try_launch(self):
        if self.scheduling_strategy == 'DFS' or self.scheduling_strategy == 'DATA' or self.scheduling_strategy == "FIFO" or self.scheduling_strategy == 'RANDOM':
            self.no_block_launch()
        elif self.scheduling_strategy == 'DHEFT' or self.scheduling_strategy == "HYBRID":
            self.block_launch()
        else:
