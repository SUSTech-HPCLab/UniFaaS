...
class DataAwareSelection:
    ...
    def select_endpoint(self, task_record, feasible_endpoints):
        ...
        for ep in feasible_endpoints:
            eft_dict[ep] = self.transfer_predictor.real_time_predict_comm_cost_for_task_record(task_record, ep)
            eft_dict[ep] += self.resource_poller.calculate_ideal_execution_duration(ep, task_record)
        return min(eft_dict, key = eft_dict.get)
    
    def _handle_data_ready(self):
        if self.data_ready_queue.qsize() > 0:
            exp_logger.info(f"handle data ready with qsize {self.data_ready_queue.qsize()}")
        while True:
            feasible_ep = []
            ...
            for key in real_time_resource.keys():
                if real_time_resource[key]['total_workers'] == 0:
                    continue
                ratio_factor = 1
                if self.endpoint_performance_ratio is not None and key in self.endpoint_performance_ratio.keys():
            ...
            if 'heft_priority' in task_record.keys():
                exp_logger.debug(f"[DHEFT] task {task_record['id']} {task_record['func_name']} with priority {task_record['heft_priority']} ")
            ...
            if task_record['status'] == States.scheduling:
                try:
                    ...
                    exp_logger.warn(f"task {task_record['id']} failed with exception {e}")
                ...
                self.resource_poller.update_status_when_submit_one_task(target_ep, task_record=task_record)
                task_record['submitted_to_poller'] = True
                task_record['status'] = States.data_managing
                self.data_manager.group_transfer(task_record)
        time.sleep(7) # sleep for 7 seconds to avoid too short interval between two scheduling
