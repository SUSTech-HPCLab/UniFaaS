...
class Scheduler:
    ...
    enable_duplicate=False,):
        ...
        if enable_execution_predictor and recorder is not None:
            self.execution_predictor = ExecutionPredictor(executors,recorder, workflow_name)
        ...
        if self.scheduling_strategy == "AUTO":
            self.auto_scheduling = AutoScheduling(self.resource_poller,self.execution_predictor,self.transfer_predictor,self.data_manager)

        exp_logger.info("[CaseStudy] Scheduling strategy: {}".format(self.scheduling_strategy))
        
        self._kill_event = threading.Event()
        self._task_schedule_thread = threading.Thread(target=self.schedule,
                                                      args=(self._kill_event,),
                                                      name="Task-Schedule-Thread")
        ...
        self._task_duplicate_thread = threading.Thread(target=self.duplicate_schedule, name="Task-Duplicate-Thread")
    ...
    def duplicate_schedule(self):
        while not self._kill_event.is_set():
            ...
            while counter > 0:
                ...
                if task_record['status'] != States.pending and task_record['status'] != States.running and task_record['status'] != States.launched:  
                    continue

                # check if important, duplicate important task
                if task_record['important'] and 'duplicated_at' not in task_record.keys():
                    min_executor = None
                    min_time = float('inf')
                    if 'predict_execution' in task_record.keys():                
                        ...
                        if min_executor != task_record['executor']:
                            ideal_increse = task_record['predict_execution'][task_record['executor']] - task_record['predict_execution'][min_executor]
                            exp_logger.info("[Duplicate] Try to duplicate important task {} from {} to {} with ideal increase {}s".format(task_record['id'], task_record['executor'], min_executor,ideal_increse))
                            ...
                            exp_logger.info(f"[Duplicate] Important task {task_record['id']} is already assigned to the best executor.")
                ...
                min_executor = None
                if cur_resource_status[cur_executor]['idle_workers'] == 0 and cur_resource_status[cur_executor]['pending_tasks'] > cur_resource_status[cur_executor]['total_workers']:
                    other_executor = []
                    ...
                    for executor in other_executor:
                        if cur_resource_status[executor]['total_workers'] == 0:
                            continue
                        if cur_resource_status[executor]['idle_workers']/cur_resource_status[executor]['total_workers'] > 0.6:
                            min_executor = executor
                            break
                if min_executor is not None:
                    exp_logger.info("[Duplicate] Duplicate task {} from {} to {}".format(task_record['id'], task_record['executor'], min_executor))
                    task_record['duplicated_at'] = min_executor
                    duplicated_task  = self.copy_duplicated_task(task_record)
                    self.submit_duplicated_task(duplicated_task)
                    self.duplicated_tasks[duplicated_task['id']] = duplicated_task
                ...
                for key1 in task_record['predict_execution']:
                    for key2 in task_record['predict_execution']:
                        tmp_cost = self.transfer_predictor.predict_comm_cost(task_record, key1, key2)
                        max_transfer_time = max(max_transfer_time, tmp_cost)

                delta_time = task_record['predict_execution'][task_record['executor']] - min_time
                ...
                if cur_idle_workers * 0.5 < cur_qsize or cur_idle_workers < cur_total_workers * 0.2:
                    self.task_duplicate_queue.put(task_record)
                    continue

                exp_logger.info("[Duplicate] Try to duplicate task {} from {} to {}".format(task_record['id'], task_record['executor'], min_executor))
                task_record['duplicated_at'] = min_executor
                duplicated_task  = self.copy_duplicated_task(task_record)
                self.submit_duplicated_task(duplicated_task)
                self.duplicated_tasks[duplicated_task['id']] = duplicated_task
    ...
    def pre_scale_out(self, cur_tasks):
        ...
        if idle_nums < cur_tasks:
            for executor in to_scale_out:
                self.executors[executor].scale_out(self.resource_poller, cur_real_time_status[executor]['max_blocks'])
        return
    ...
    def put_task_to_data_ready_queue(self, task_record):
        for executor in self.resources.keys():
            task_with_transfer_size = TaskWithTransferSize(task_record, executor, self.transfer_predictor)
    ...
    def _DATA_handle_ready(self):
        ...
        for ep in cur_resource.keys():
            redundant_cap = 0 
            idle_workers.append(max(0,cur_resource[ep]['idle_workers']-cur_resource[ep]['launch_queue_task_num'] + redundant_cap))
            endpoints.append(ep)
    ...
    def _FIFO_handle_ready(self):
        ...
        for ep in cur_resource.keys():
            ...
            idle_workers_info[ep] = max(0,cur_resource[ep]['idle_workers']-cur_resource[ep]['launch_queue_task_num']+redundant_cap)
    ...
    def random_schedule(self):
        ...
        time_interval = 5
        duration = 0
        exp_logger.debug(f"[MicroExp] now time {time.time()} and last ideal time {graphHelper.get_task_idle_since()}")

        if cur_qsize >= batch_size or time.time() - graphHelper.get_task_idle_since() > time_interval:
            starting_time = time.time()
            exp_logger.debug(f"[MicroExp] start random based scheduling at time {time.time()}")
            cur_resource = self.resource_poller.get_real_time_status()
            ...
            while cur_qsize > 0:      
                ...
                if task_record['status'] == States.scheduling:
                    ...
                    if int(task_record['id']) % 100 == 1:
                        exp_logger.debug(f"[MicroExp] end scheduling, task {task_record['id']} at time {time.time()}")
                cur_qsize -= 1
            duration = time.time() - starting_time
        exp_logger.debug(f"[MicroExp] end once scheduling at time {time.time()} with duration1 {duration}")
    ...
    def dfs_based_schedule(self):
        ...
        duration = 0
        if cur_qsize >= batch_size or time.time() - graphHelper.get_task_idle_since() > time_interval:
            exp_logger.debug(f"[MicroExp] start dfs based scheduling at time {time.time()}")
            starting_time = time.time()
            ...
            partition_res = self.partition_by_ideal_worker_nums(cur_qsize, resource_rank)
            if cur_qsize > 0 :
            ...
            while 0 < entry_qsize :
                ...
                cur_vis = self.dfs_on_raw_graph(head_fu, cur_vis ,resource_rank,partition_res)
                entry_qsize -= 1
            while cur_qsize > 0:
                task_record = graphHelper.task_queue.get()
                ...
                cur_qsize -= 1
            duration = time.time() - starting_time
        exp_logger.debug(f"[MicroExp] end once scheduling at time {time.time()} with duration {duration}")
        time.sleep(max(0,time_interval-duration))
    ...
    def _stat_eft_info_before_managing(self, task_record):        
        ...
        if "predict_execution" not in task_record.keys():
            self.execution_predictor.predict(task_record) #predict_execution, predict_output
        
        est = self.transfer_predictor.predict_est_for_certain_executor(task_record, target_executor)
        cur_time = time.time()
        if est == 0:
            est = cur_time
        dispatch_latency = 3
        ...
        if cur_size < self.ideal_workers[target_executor]:  
            ...
            self.eft_queue[target_executor].put(max(earliest_ft,est) + dispatch_latency + execution_time)
            task_record['predict_eft'] = max(earliest_ft,est) + dispatch_latency + execution_time

    def advance_schedule(self):
        batch_size = 5000
        cur_qsize = graphHelper.task_queue.qsize()
        
        if cur_qsize >= batch_size or time.time() - graphHelper.get_task_idle_since() > 5:
            # rank the resource by the number of ideal max workers
            cur_resource = self.resource_poller.get_real_time_status()
            resource_rank = list(cur_resource.keys())
            def get_ideal_worker_nums(label):
    ...
    def partition_by_ideal_worker_nums(self, tot_tasks, resource_rank):
        ...
        partition_res = [ math.ceil(tot_tasks*worker/tot_workers)for worker in ideal_worker_nums]
    ...
    def dfs_on_raw_graph(self, cur_node,cur_vis, resource_rank, partition_res):
        ...
        exp_logger.debug(f"{task_record['id']} is assigned to {task_record['executor']} with {task_record['modified_times']}")
        if int(task_record['id']) % 100 == 1:
            exp_logger.debug(f"[MicroExp] end scheduling, task {task_record['id']} at time {time.time()}")
        ...
        if cur_node in graphHelper.raw_graph.keys():
            for val in graphHelper.raw_graph[cur_node]:
                cur_vis = self.dfs_on_raw_graph(val,cur_vis, resource_rank, partition_res)
        return cur_vis
                ...
                with child['task_launch_lock']:
                    if child['status'] == States.dynamic_adjust:
                        if "predict_execution" not in child.keys():
                            self.execution_predictor.predict(child) #predict_execution, predict_output
                        resource = self.resource_poller.get_real_time_status()
                        if resource[child['executor']]['idle_workers'] <= 0 and resource[child['executor']]['pending_tasks'] >= 1:
                            self._advance_relocate_task(child)
                        self._stat_eft_info_before_managing(child)
                        child['status'] = States.data_managing
                    self.data_manager.handle_task_record_data_managing(child, input_data=task_res, invoke=True)
        elif self.scheduling_strategy == "HEFT" or self.dynamic_adjust_strategy == "GREEDY":
            appfu = task_record['app_fu']
            child_task_fu_list = self.raw_graph[appfu]
            child_task_list = [self.fu_to_task[fu] for fu in child_task_fu_list]
            for child in child_task_list:
    ...
    def _advance_relocate_task(self, task_record):
        ...
        cur_time = time.time()
        estimated_est = self.eft_queue[original_exe].get() if not self.eft_queue[original_exe].empty() else cur_time
        if estimated_est is not cur_time:
            self.eft_queue[original_exe].put(estimated_est)
        estimated_eft = max(estimated_est, self.transfer_predictor.predict_est_for_certain_executor(task_record, original_exe)) + task_record['predict_execution'][original_exe] + dispatch_latency
        resource = self.resource_poller.get_real_time_status()
        # find the best resource
        possible_resource = []
        for key in resource.keys():
            if resource[key]['idle_workers'] > 0 and resource[key]['pending_tasks']<= 0:
        ...
        for resource in possible_resource:
            est = max(cur_time, self.transfer_predictor.predict_est_for_certain_executor(task_record, resource))
        ...
        logging.info("Relocate task {} from {} to {}".format(task_record['id'], original_exe, changed_resource))
        exp_logger.info("Relocate task {} from {} to {}".format(task_record['id'], original_exe, changed_resource))
