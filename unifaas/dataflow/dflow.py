...
from unifaas.dataflow.error import BadCheckpoint, ConfigurationError, DependencyError, DuplicateTaskError
...
class DataFlowKernel(object):
    ...
    def __init__(self, config=Config()):
        ...
        if config.initialize_logging:
            unifaas.set_file_logger("{}/unifaas.log".format(self.run_dir), name="unifaas", level=logging.INFO)
            unifaas.set_file_logger("{}/exp.log".format(self.run_dir),name="experiment", level=logging.INFO)
        ...
        self._task_launch_thread = threading.Thread(target=self.launch_in_period,
        ...
        self._task_states_report_thread = threading.Thread(target=self.report_task_states_in_period,
        ...
        self.data_trans_management = DataTransferManager(self.executors, password_file=config.password_file, 
            bandwith_info=config.bandwidth_info, transfer_type=config.transfer_type)
        ...
        if self.enable_schedule:
            self.task_launcher = TaskLauncher(self.executors, self.launch_if_ready,config.scheduling_strategy,self.task_status_tracker)
            self._task_launch_thread.start()
            self._task_states_report_thread.start()
            self.scheduler = Scheduler(self.executors, 
    ...
    def handle_exec_update(self, task_record, future, duplicated=False):
        ...
        if 'original_task' in task_record.keys():
            ...
            if original_task['status'] == States.exec_done:
                ...
                exp_logger.info(f"[Duplicate] Original task {original_task['id']} is replaced by task {task_id}")
        ...
        try:
            ...
            if isinstance(res, dict):
                ...
                exp_logger.debug(f"[MicroExp] Task {task_record['id']} completed at {time.time()}")
                if self.enable_execution_recorder:
                    predict_time = 0
                    if 'predict_execution' in task_record.keys():
                        executor = task_record['executor'] if duplicated == False else task_record['duplicated_at']
                ...
                exp_logger.info(f"[CaseStudy] Task {task_record['id']} completed: {res['execution_time']}|{res['cpu_percent']}|{res['output_size']}")
                if duplicated == False:
                    self.status_poller.update_from_result_handler(res, task_record["executor"], task_record)
                else:
                    self.status_poller.update_from_result_handler(res, task_record["duplicated_at"], task_record)
                res = res['result']
                self.task_status_tracker.update_when_task_done(task_record)
    ...
    def launch_if_ready(self, task_record):
        ...
        or callback.
        """
        if not task_record['status'] == States.pending and not task_record['status'] == States.queuing :
            return
        ...
        if self._count_deps(task_record['depends']) == 0:
            ...
            if not exceptions_tids:
                ...
                with task_record['task_launch_lock']:
                    if task_record['status'] == States.pending or task_record['status'] == States.queuing:
                        try:
                            if not task_record['submitted_to_poller']:
                                self.status_poller.update_status_when_submit_one_task(task_record['executor'])
                                task_record['submitted_to_poller'] = True
                            exp_logger.debug(f"[MicroExp] Task {task_record['id']} launch_task on executor at time {time.time()}")
                            self.scheduler.put_important_task_into_duplicated_queue(task_record)
                            self.task_status_tracker.update_when_task_submit_to_executor(task_record)
    ...
    def launch_task(self, task_record, executable, *args, **kwargs):
        ...
        with self.submitter_lock:
            exp_logger.info(f"[CaseStudy] Task {task_record['id']} launch: {task_record['executor']}|{task_record['func_name']}")
    ...
    def submit(self, func, app_args, executors='all', cache=False, ignore_for_cache=None, app_kwargs={}, join=False, never_change=False):
        ...
        exp_logger.debug(f"[MicroExp] Task {task_id} submitted on executor at time {time.time()}")
        ...
        logger.debug("Task {} submitted for App {}, {}".format(task_id,
                                                              task_def['func_name'],
                                                              waiting_message))
        ...
        if self.enable_schedule:
            ...
            logger.debug("Task {} submitted for App {}, {}".format(task_id,task_def['func_name'],waiting_message))
        ...
        while not kill_event.is_set():
            ...
            for task_id in list(self.duplicated_tasks):
                if task_id not in self.duplicated_tasks:
                    logger.debug("Task {} no longer in duplicated task list".format(task_id))
                else:
