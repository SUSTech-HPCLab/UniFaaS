...
class ResourceStatusPoller(object):
    ...
    def __init__(self, executors, poll_period=10):
        if ResourceStatusPoller.__first_init == False:
            ...
            self._status_poller_thread = threading.Thread(target=self.update_from_webserivce,
                                                      args=(self._kill_event,),
                                                      name="Status-Poller-Thread")
            self._status_poller_thread.daemon = True
            self._status_poller_thread.start()
            self._status_monitor_thread = threading.Thread(target=self.monitor_resource,
                                                      args=(self._kill_event,),
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as thread_executor:
            for funcx_executor in self.executors:
                if isinstance(funcx_executor, FuncXExecutor):
                    futures_dict[funcx_executor.label] = None if funcx_executor.fxc is None  \
                        else thread_executor.submit(funcx_executor.fxc.get_endpoint_status, funcx_executor.endpoint)
        
        for executor in futures_dict.keys():
            try:
                data = futures_dict[executor].result() if futures_dict[executor] is not None else None
            except Exception as e:
                logger.warning(f"Can't get Executor {executor} status due to exception {e} at result")
                continue
            if data is None:
            ...
            if data.get('logs') is not None:
                    ...
                    if len(logs) > 0:
                        ...
                        exp_logger.info(f"update_from_service {executor}|{info['total_workers']}|{info['idle_workers']}|{info['pending_tasks']}")      
                        if info['total_workers'] == 0:
                            ...
                            if info['total_workers'] > self.real_time_status[executor]['total_workers']:
                                self.update_when_scaling_out(executor, info['total_workers'])
                            elif info['total_workers'] < self.real_time_status[executor]['total_workers']:
                                self.update_when_scaling_in(executor, info['total_workers'])
                        exp_logger.info(f"[RealTimeWorkerInfo] {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}|{self.real_time_status[executor]['launch_queue_task_num']}")
    ...
    def update_real_time_status_when_closed(self, executor):
        ...
        self.real_time_status[executor]['submit_counter'] = 0
        self.real_time_status[executor]['launch_queue_task_num'] = 0
        exp_logger.info(f"[RealTimeWorkerInfo] close {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}|{self.real_time_status[executor]['launch_queue_task_num']}")
    ...
    def update_real_time_status_when_open(self, executor, info):
        ...
        self.real_time_status[executor]['pending_tasks'] = 0
        exp_logger.info(f"[RealTimeWorkerInfo] open {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}|{self.real_time_status[executor]['launch_queue_task_num']}")
        self.real_time_status[executor]['idle_workers'] = max(0, self.real_time_status[executor]['total_workers'] -self.real_time_status[executor]['submit_counter'])
        self.real_time_status[executor]['pending_tasks'] = max(0, self.real_time_status[executor]['submit_counter'] - info["total_workers"])
        self.real_time_status[executor]['submit_counter'] = 0
        exp_logger.info(f"[RealTimeWorkerInfo] handle counter {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}|{self.real_time_status[executor]['launch_queue_task_num']}")


    def update_when_launch_task(self, executor, task_record=None):
        self.real_time_status[executor]['launch_queue_task_num'] -= 1
        if self.real_time_status[executor]['pending_tasks'] > 0 or self.real_time_status[executor]['idle_workers'] <= 0:
    ...
    def release_being_executed_time(self, executor, task_record):
        if 'predict_execution' in task_record.keys():
            self.cur_being_executed_time[executor] -= task_record['predict_execution'][executor]
    ...
    def update_status_when_submit_one_task(self, executor, task_record=None):
        ...
        if self.real_time_status[executor]['closed']:
            ...
            launch_queue_task_num = self.real_time_status[executor]['launch_queue_task_num']
            exp_logger.info(f"[RealTimeWorkerInfo] after submit {task_str} {executor}|{total_workers}|{idle_workers}|{pending_tasks}|{launch_queue_task_num}")


    def calculate_ideal_execution_duration(self, executor, task_record):
        if self.real_time_status[executor]['total_workers'] == 0:
            return float('inf')
        return self.cur_being_executed_time[executor] / self.real_time_status[executor]['total_workers'] + task_record['predict_execution'][executor]
    ...
    def update_from_result_handler(self, result, executor, task_record):
        ...
        if self.real_time_status[executor]['closed']:
            ...
            exp_logger.info(f"[RealTimeWorkerInfo] open by result {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}")
            self.real_time_status[executor]['submit_counter'] = max(0, self.real_time_status[executor]['submit_counter'] - 1) # since one task is completed
            self.real_time_status[executor]['total_workers'] = result["total_workers"]
            self.real_time_status[executor]['idle_workers'] = max(0, self.real_time_status[executor]['total_workers'] -self.real_time_status[executor]['submit_counter'])
            self.real_time_status[executor]['pending_tasks'] = max(0, self.real_time_status[executor]['submit_counter'] - result["total_workers"])
            self.real_time_status[executor]['submit_counter'] = 0
            exp_logger.info(f"[RealTimeWorkerInfo] handle counter{executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}")
        else:
            if result["total_workers"] > self.real_time_status[executor]['total_workers']:
                exp_logger.info(f"[RealTimeWorkerInfo] update_when_scaling_out result worker {result['total_workers']}, total worker {self.real_time_status[executor]['total_workers']}")
                self.update_when_scaling_out(executor, result["total_workers"])
            elif result["total_workers"] < self.real_time_status[executor]['total_workers']:
                exp_logger.info(f"[RealTimeWorkerInfo] update_when_scaling_in result worker {result['total_workers'] }, total worker {self.real_time_status[executor]['total_workers']}")
                self.update_when_scaling_in(executor, result["total_workers"])

            if self.real_time_status[executor]['pending_tasks'] > 0:
                self.real_time_status[executor]['pending_tasks'] -= 1
            else:
                if self.real_time_status[executor]['idle_workers'] + 1 <= self.real_time_status[executor]['total_workers']:
                    self.real_time_status[executor]['idle_workers'] += 1 
            exp_logger.info(f"[RealTimeWorkerInfo] update by result {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}|{self.real_time_status[executor]['launch_queue_task_num']}")
    ...
    def update_when_scaling_out(self, executor, cur_workers):
        ...
        cur_idle_workers = max(0,cur_workers - (original_pending_tasks + (original_workers - original_idle_workers)))
        cur_pending_tasks = max(0, original_pending_tasks + (original_workers - original_idle_workers) - (cur_workers - cur_idle_workers))
    ...
    def update_when_scaling_in(self, executor, cur_workers):
       ...
       # cur_pending_tasks = max(0, original_pending_tasks - (original_workers - original_idle_workers) + (original_workers - cur_workers))
        ...
        if cur_idle_workers == 0:
            self.real_time_status[executor]['pending_tasks'] += max(0, decrease_workers-original_idle_workers)
    ...
    def finish_n_tasks(self, info, executor, n):
        ...
        ****Deprecated****
        ...
        if 'cpu_freqs_max' in info.keys() and 'cpu_freqs_min' in info.keys() and 'cpu_freqs_current' in info.keys():
            cpu_max = max(info['cpu_freqs_max'], info['cpu_freqs_min'], info['cpu_freqs_current'])
        ...
        if self.real_time_status[executor]['closed']:
            ...
            self.real_time_status[executor]['pending_tasks'] = 0
            self.real_time_status[executor]['idle_workers'] = info["total_workers"]
            exp_logger.info(f"[RealTimeWorkerInfo] {executor}|{self.real_time_status[executor]['total_workers']}|{self.real_time_status[executor]['idle_workers']}|{self.real_time_status[executor]['pending_tasks']}")
            self.real_time_status[executor]['submit_counter'] = max(0, self.real_time_status[executor]['submit_counter'] - n) # since one task is completed
            self.real_time_status[executor]['total_workers'] = info["total_workers"]
            self.real_time_status[executor]['idle_workers'] = max(0, self.real_time_status[executor]['total_workers'] -self.real_time_status[executor]['submit_counter'])
            self.real_time_status[executor]['pending_tasks'] = max(0, self.real_time_status[executor]['submit_counter'] - info["total_workers"])
            self.real_time_status[executor]['submit_counter'] = 0
            ...
            if n > self.real_time_status[executor]['pending_tasks']:
                original_pending_tasks = self.real_time_status[executor]['pending_tasks']
                self.real_time_status[executor]['pending_tasks'] = 0
                self.real_time_status[executor]['idle_workers'] += n - original_pending_tasks
            else:
    ...
    def _cal_ideal_upper_bound_of_workers(self):
        for key in self.resource_status.keys():
            cores_per_unit = min(self.resource_status[key]["max_workers_per_node"], self.resource_status[key]["cores_per_unit"])
            self.resource_status[key]["max_cores"] = cores_per_unit \
