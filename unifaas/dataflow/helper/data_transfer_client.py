...
class RsyncTransferClient:
    ...
    def __init__(self, executors, password_file=None, transfer_record_dir=None, passwordless=True):
    ...
    def generate_transfer_command(self,src_ip, src_user, dest_ip, dest_user, src_path,dest_path):
    ...
    def generate_passwordless_command(self,src_ip, src_user, dest_ip, dest_user, src_path,dest_path):
        h1_address = f"{src_user}@{src_ip}"
        h2_address = f"{dest_user}@{dest_ip}"
    ...
    def transfer(self, file, remote_host):
        if isinstance(file, RemoteFile) or isinstance(file, RemoteDirectory):
            ...
            if remote_host not in self.executor_address_info.keys():
                raise Exception("Transfer error! Can't find the remote host at executor address dict")
            ...
            if self.passwordless:
                cmd = self.generate_passwordless_command(rsync_ip, rsync_username,dest_ip,dest_user,src_path,dest_path)
            else:
                cmd = self.generate_transfer_command(rsync_ip, rsync_username,dest_ip,dest_user,src_path,dest_path)
            ...
            if isinstance(file, RemoteFile):
                ...
                if dest_ip == rsync_ip and dest_user == rsync_username and src_dir == dest_path:
                    task = {'task_id': task_id, 'status': 'LOCAL', 'future': None, 'stdout': None, 'stderr': None}
            ...
            future = self.transfer_thread_pool.submit(subprocess.run, cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            task = {'task_id': task_id, 'status': 'ACTIVE', 'future': future, 'stdout': None, 'stderr': None, 'transfer_times':0, 'start_time': time.time()}
            task['remote_file'] = file
    ...
    def _analyze_std_out(self, out_info):
        ...
        speed_pattern = r"\d[^a-z]*bytes\/sec" # search the string like 140.00 bytes/sec
        ...
        speed_num = re.findall(r"[\d.,]+", speed_str)[0] # conatins comma, like 140,000.00
...
class SFTPClient:
    ...
    def __init__(self, executors, command_server_port=12548, transfer_record_dir=None,):
        self.transfer_recorder = TransferRecorder(record_dir=transfer_record_dir)
        self.resource_poller = ResourceStatusPoller(executors=executors)
        self.executors = executors
        ...
        self.command_server_port = command_server_port # command server on the endpoint. default is 12548
        self.executor_address_info = {}
        self.transfer_tasks = Queue()
    ...
    def generate_transfer_command(self,src_ip, src_user, dest_ip, dest_user, src_path,dest_path):
        cmd = f"TRANSFER|{src_user}@{src_ip}|{src_path}|{dest_user}@{dest_ip}|{dest_path}"
        return cmd
    ...
    def submit_one_task(self, src_host, dest_host):
        ...
        future = self.thread_pool[src_host][dest_host].submit(self.send_transfer_command, transfer_cmd, src_ip)
        task['future'] = future
        future.add_done_callback(partial(self._transfer_done, task=task))
    ...
    def transfer(self, file, remote_host):
        if isinstance(file, RemoteFile) or isinstance(file, RemoteDirectory):
            ...
            dest_path = f"{dest_path}{basename}" if dest_path.endswith("/") else f"{dest_path}/{basename}"
            
            transfer_cmd = self.generate_transfer_command(src_ip, src_username,dest_ip,dest_user,src_path,dest_path)
            task_id = str(uuid.uuid4())

            #check the same dest and source only support remotefile for now
            if isinstance(file, RemoteFile):
                if dest_ip == src_ip and dest_user == dest_user and src_path == dest_path:
                    task = {'task_id': task_id, 'status': 'LOCAL', 'future': None, 'stdout': None, 'stderr': None}
                    task['remote_file'] = file
            ...
            task = {'task_id': task_id, 'status': 'ACTIVE', 'future': None, 'stdout': None, 'stderr': None, 'transfer_times':0, 'start_time': time.time()}
            task['remote_file'] = file
            task["src_ep"] = f"{src_username}@{src_ip}"
            task["dst_ep"] = f"{dest_user}@{dest_ip}"
            task["dest_path"] = dest_path
            ...
            if file in self.transfer_history_tbl.keys():
                if remote_host in self.transfer_history_tbl[file]:
                    ...
                    if exist_task['status'] == 'SUCCEEDED' or exist_task['status'] == 'LOCAL':
                        task['status'] = 'LOCAL'
                        logger.info(f"[SFTP] Skip transfer task, due to history marked with local: {task}")
                        return task
                    return exist_task
    ...
    def _transfer_done(self, future, task):
        try:
            ...
            if transfer_result.startswith("SUCCESS"):
                ...
                self.transfer_recorder.write_record(task["src_ep"], task["dst_ep"], speed, transfer_size, transfer_time, task['total_time'], task['prediction_time'])  
            else:
    ...
    def _retry_transfer(self, task):
        ...
        future = self.retry_submission.submit(self.send_transfer_command, transfer_cmd, src_ip)
        task['future'] = future
        future.add_done_callback(partial(self._transfer_done, task=task))
...
class GlobusTransferClient:
    ...
    def __init__(
        ...
        self._track_globus_transfer_thread = threading.Thread(target=self._track_globus_transfer_status,
    ...
    def transfer(self, file, remote_host):
        if isinstance(file, GlobusFile):
            ...
            basename = file.file_name
            target_path = f"{dest_path}{basename}" if dest_path.endswith("/") else f"{dest_path}/{basename}"
            if dest_ep == file.endpoint:
                basename = file.file_name
                if target_path == file.file_path:
                    task = {'task_id': unifaas_transfer_task_id, 'status': 'LOCAL', 'future': None, 'stdout': None, 'stderr': None}
                    task['remote_file'] = file
            ...
            for exe in self.executor_address_info.keys():
                if self.executor_address_info[exe]["globus_ep_id"] == file.endpoint:
                    src_host = exe
                    break
            task = {'task_id': unifaas_transfer_task_id, 'status': 'ACTIVE', 'future': None, 'stdout': None, 'stderr': None, 'transfer_times':0, 'start_time': time.time()}    
            task['remote_file'] = file
            task["src_ep"] = file.endpoint
            task["dst_ep"] = dest_ep
            task["dest_path"] = target_path
            ...
            if file in self.transfer_history_tbl.keys():
                if remote_host in self.transfer_history_tbl[file]:
                    logger.info(f"[GLOBUS] Skip transfer task, due to already in history")
                    exist_task = self.transfer_history_tbl[file][remote_host]
                    if exist_task['status'] == 'SUCCEEDED' or exist_task['status'] == 'LOCAL':
                        task['status'] = 'LOCAL'
                        logger.info(f"[GLOBUS] Skip transfer task, due to history marked with local: {task}")
                        return task
                    return exist_task
            if task['remote_file'] not in self.transfer_history_tbl.keys():
                self.transfer_history_tbl[task['remote_file']] = {}
    ...
    def globus_transfer(self, transfer_file: RemoteFile, dest_ep, dest_path, recursive=False):
        ...
        try:
            ...
            print(
                "Globus transfer from {}{} to {}{} failed due to error: {}".format(
                    transfer_file.endpoint, transfer_file.file_path, dest_ep, dest_path, e
                )
            )
            raise Exception(
                "Globus transfer from {}{} to {}{} failed due to error: {}".format(
        ...
        while not kill_event.is_set():
            ...
            for task_id, unifaas_transfer_task in list(self.flying_tasks.items()):
                ...
                if transfer_status["status"] == "SUCCEEDED":
                    ...
                    self.transferring_size[src_host][dest_host] -= unifaas_transfer_task['file_size']
                    self.transferring_file_num[src_host][dest_host] -= 1
    ...
    def _retry_transfer(self, unifaas_task):
        ...
        globus_task = self.globus_transfer(unifaas_task['remote_file'], unifaas_task['dst_ep'], unifaas_task['dest_path'])
