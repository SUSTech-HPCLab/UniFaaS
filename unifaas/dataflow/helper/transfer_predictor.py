...
class TransferPredictor:
    ...
    def _init_bandwith_info(self, bandwith_info=None):
        ...
        if bandwith_info is None:
            ...
            for fx_exe in funcx_executor:
                prob_list = self.probe_dic[
                    fx_exe
                ].result()  # result is a dict, with execution info
                if isinstance(prob_list, dict):
                    prob_list = prob_list[
                        "result"
                    ]  # prob_list is a list of remote file
    ...
    def perdict_based_on_bandwith(
        self, from_exe, to_exe, transfer_size, num_workers=9, latency=1
    ):
        """
    ...
    def predict_est_for_certain_executor(self, task_record, dest_executor):
        ...
        for data in data_to_trans:
            ...
            if start_executor is None:
                start_executor = (
                    "data_pool"  # the remote file not belong to any executor/ep
                )
            est_list.append(
                cur_time
                + net_latency
                + self.perdict_based_on_bandwith(
                    start_executor, dest_executor, data.file_size
                )
            )
        ...
        for dep in task_record["depends"]:
            ...
            est_list.append(
                ...
                + net_latency
                + self.perdict_based_on_bandwith(
                    start_executor, dest_executor, data_size
                )
            )
    ...
    def predict_avg_comm_cost(self, input_size):
        ...
        return res / (
            len(self.network_info.keys()) * (len(self.network_info.keys()) - 1)
        )
    ...
    def real_time_predict_for_transfer(
        ...
        bandwith_to_bytes = bandwith * 1024 * 1024
        pure_trans_time = (
            cur_transffering_size / bandwith_to_bytes
    ...
    def real_time_predict_comm_cost_for_task_record(self, task_record, ep):
        ...
        for src_ep in transfer_size_cnt.keys():
            if transfer_size_cnt[src_ep] > 0:
                transfer_cost = max(
                    transfer_cost,
                    self.real_time_predict_for_transfer(
                        transfer_size_cnt[src_ep], src_ep, ep
                    ),
                )
    ...
    def predict_init_comm_cost(self, task_record, ep):
        ...
        data_transfer_times = [
            self.perdict_based_on_bandwith(
                self.find_target_ep(data), ep, data.file_size
            )
            for data in data_to_trans
        ]
