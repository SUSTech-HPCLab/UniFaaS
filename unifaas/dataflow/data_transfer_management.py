...
class DataTransferManager(object):
    ...
    def __init__(self, executors, transfer_type='rsync', password_file=None, bandwith_info=None):
        if DataTransferManager.__first_init is False:
            self.entry_task_queue = Queue()
            self.executors = executors
            self.dummy_endpoint_info_dir = os.path.join(UNIFAAS_HOME, "dummy_endpoint_info")
            self.label_to_executor = {}
            self.executor_to_globus = {}
            self.executor_to_data_path = {}
            if transfer_type=="rsync":
                self.dtc = RsyncTransferClient(self.executors,password_file=password_file)
            elif transfer_type=="sftp":
                self.dtc = SFTPClient(self.executors)
            else:
                self.dtc = GlobusTransferClient(self.executors)
            self.data_transfer_predictor = TransferPredictor(self.executors, bandwith_info=bandwith_info, dtc=self.dtc)
            ...
            self._track_transfer_thread = threading.Thread(target=self._track_transfer_status,
                                                        args=(self._kill_event,),
                                                        name="Data-Manage-Thread")
            self._track_transfer_thread.daemon = True
            self._track_transfer_thread.start()

            self._handle_not_scheduling_thread = threading.Thread(target=self._handle_not_scheduling_task, 
                                                                args=(self._kill_event,),
    ...
    def _analyze_data_to_transfer(self, task_record):
        ...
        # list concat
        data_to_trans = DataTransferManager.check_data_transfer(args) + \
    ...
    def _set_pending_for_group_transfer(self, task_record):
        ...
        if 'finish_group_transfer' in task_record.keys() and task_record['finish_group_transfer']:
    ...
    def group_transfer(self, task_record):
        ...
        new_args, kwargs = self.sanitize_and_wrap(task_record['args'], task_record['kwargs'])
        data_to_trans = DataTransferManager.check_data_transfer(new_args) + \
        DataTransferManager.check_data_transfer(kwargs)
        ...
        with task_record['task_data_trans_lock']:
            for remote_file in data_to_trans:
                ...
                if trans_info['status'] == 'LOCAL':
                    ...
                    logger.info(f"[CaseStudy] Transfer for Task {task_record['id']} with file {trans_info['dest_path']}")
                    if task_record['data_trans_times'] == 0:
                        task_record['finish_group_transfer'] = True
                        self._set_pending_for_group_transfer(task_record)
                    continue
                else:
                    trans_info['prediction_time'] = self.data_transfer_predictor.real_time_predict_for_transfer(0,trans_info['src_host'], trans_info['dest_host'])
                    if trans_info['task_id'] not in self.group_trans_to_record:
                        self.group_trans_to_record[trans_info['task_id']] = []
                    self.group_trans_to_record[trans_info['task_id']].append(task_record)
    ...
    def handle_task_record_data_managing(self, task_record, input_data=None, retry=False, invoke=False):
        if task_record['status'] == States.pending:
            return
        
        exp_logger.debug(f"[MicroExp] start data transfer management, task {task_record['id']} at time {time.time()}")
        ...
        if task_record['id'] not in self.trans_count_in_param.keys():
            ...
            with task_record['task_data_trans_lock']:
                ...
                logger.debug(f"Task {task_record['id']} data_trans_times plus {len(data_to_trans_in_param)} by param, now {task_record['data_trans_times']}")
                exp_logger.debug(f"Task {task_record['id']} data_trans_times plus {len(data_to_trans_in_param)} by param, now {task_record['data_trans_times']}")
            self.trans_count_in_param[task_record['id']] = data_to_trans_in_param

        if not task_record['status'] == States.data_managing and not task_record['status'] == States.dynamic_adjust:
        ...
        if invoke:
            ...
            with task_record['task_data_trans_lock']:
                if input_data is not None:
                    ...
                    task_record['data_trans_times'] += len(data_to_trans)
                    logger.debug(f"Task {task_record['id']} data_trans_times plus {len(data_to_trans)} by input_data, now {task_record['data_trans_times']}")
                    exp_logger.debug(f"Task {task_record['id']} data_trans_times plus {len(data_to_trans)} by input_data, now {task_record['data_trans_times']}")
        ...
        with task_record['task_data_trans_lock']:
            ...
            if input_data is not None:
                data_to_trans = data_to_trans + DataTransferManager.check_data_transfer(input_data)
            ...
            for remote_file in data_to_trans:
                ...
                # if the transfer is local, we don't need to track it, just decrease
                if trans_info['status'] == 'LOCAL':
                    exp_logger.debug(f"Transfer local sucess Task{task_record['id']}|{trans_info['task_id']}|{trans_info['src_ep']}|{trans_info['dst_ep']}|{task_record['executor']}")
                    logger.info(f"[CaseStudy] Transfer for Task {task_record['id']} with file {trans_info['dest_path']}")
                    task_record['data_trans_times'] -= 1
                    logger.debug(f"Task {task_record['id']} data_trans_times reduce 1 by local, now {task_record['data_trans_times']}")
                    exp_logger.debug(f"Task {task_record['id']} data_trans_times reduce 1 by local, now {task_record['data_trans_times']}")
                    self._set_pending_if_ready(task_record)
                    continue
                else:
                    exp_logger.debug(f"Task {task_record['id']}  submit data_trans with Task{trans_info['task_id']}")
    ...
    def _set_pending_if_ready(self, task_record):
        ...
        dep_count = DataTransferManager._count_dep_by_param(task_record['args']) + \
        DataTransferManager._count_dep_by_param(task_record['kwargs'])
        logger.debug(f"_set_pending_if_ready Task {task_record['id']} dep_count {dep_count} data_trans_times {task_record['data_trans_times']}")
        exp_logger.debug(f"_set_pending_if_ready Task {task_record['id']} dep_count {dep_count} data_trans_times {task_record['data_trans_times']}")
        # handle_managing_times is used to avoid the task is set to pending before the transfer is submitted
        if task_record['data_trans_times'] == 0 and dep_count == 0 and task_record['handle_managing_times'] >= len(task_record['depends']): 
            task_record['status'] = States.pending
            exp_logger.debug(f"[MicroExp] Task {task_record['id']} set to pending at {time.time()}")
            if task_record['id'] in self.trans_count_in_param.keys(): 
                self.trans_count_in_param.pop(task_record['id'])
        if "confirm_deps_done" in task_record.keys() and task_record["confirm_deps_done"]:
            ...
            for i in range(qsize):
                ...
                if info['status'] == 'SUCCEEDED':
                    ...
                    if task_id in self.group_trans_to_record.keys():
                        for task_record in self.group_trans_to_record[task_id]:
                            ...
                            if 'speed' in info.keys():
                                exp_logger.info(f"[CaseStudy] Transfer sucess Task {task_record['id']}|{info['task_id']}|{info['src_ep']}|{info['dst_ep']}|{info['speed']}|{info['transfer_size']}")
                            logger.info(f"[CaseStudy] Transfer for Task {task_record['id']} with file {info['dest_path']}")
                    ...
                    if task_id not in self.active_transfers.keys():
                        ...
                        self.dtc.transfer_tasks.put(info)  # this thread is ahead of the thread that push the task to active_transfers
                        continue
                    task_record = self.active_transfers[task_id]
                    if 'speed' in info.keys():
                        exp_logger.info(f"Transfer sucess Task{task_record['id']}|{info['task_id']}|{info['src_ep']}|{info['dst_ep']}|{info['speed']}|{info['transfer_size']}")
                    with task_record['task_data_trans_lock']:
                        task_record['data_trans_times'] -= 1
                    self._set_pending_if_ready(task_record)
                    if task_id in self.active_transfers.keys()  :
                        exp_logger.info(f"Transfe task_id {task_id} was poped from active_transfers")
    ...
    def _count_dep_fu_without_transfer(task_record):
        ...
        for dep in task_record['depends']:
            if dep.done():
                result = dep.result()
                if not isinstance(result, RemoteFile) and not isinstance(result, RemoteDirectory):
                    dep_count += 1
        return dep_count
        ...
        while not kill_event.is_set():
            if self.not_scheduling_task_queue.qsize() > 0:
                ...
                for i in range(cur_size):
                    ...
                    if task_record['status'] == States.data_managing or task_record['status'] == States.dynamic_adjust:
                        self.handle_task_record_data_managing(task_record, input_data, retry=True, invoke=True)
                    else:
                        self.not_scheduling_task_queue.put((task_record, input_data))
                        continue
            time.sleep(0.1)
